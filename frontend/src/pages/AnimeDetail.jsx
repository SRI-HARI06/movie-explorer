...
function AnimeDetail() {
    ...
    useEffect(() => {
        API.get(`/movies/anime-detail/${malId}`).then(res => {
            setAnime(res.data)
            setLoading(false)
    ...
    const addToWatchlist = async () => {
        ...
        try {
            await API.post('/watchlist', {
                imdb_id: String(anime.mal_id),
                title: anime.title,
                poster_url: anime.poster_path,
                genre: anime.genre,
    ...
    if (!anime) return <div style={{ textAlign: 'center', padding: '100px', color: '#555' }}>Anime not found!</div>
        ...
        <div style={{ maxWidth: '1100px', margin: '30px auto', padding: '0 20px' }}>
            ...
            <div style={{ display: 'flex', gap: '40px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '35px', marginBottom: '30px' }}>
                <div style={{ flexShrink: 0 }}>
                    {anime.poster_path ? (
                        <img src={anime.poster_path} alt={anime.title} style={{ width: '220px', borderRadius: '14px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }} />
                    ) : (
                        <div style={{ width: '220px', height: '320px', background: 'rgba(124,58,237,0.2)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '4rem' }}>⛩️</div>
                    )}
                </div>
                <div style={{ flex: 1 }}>
                    <h1 style={{ fontSize: '2rem', fontWeight: '700', background: 'linear-gradient(135deg, #a78bfa, #60a5fa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '15px' }}>{anime.title}</h1>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>📅 Year: {anime.release_year || 'N/A'}</p>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>🎭 Genre: {anime.genre || 'N/A'}</p>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>🎬 Episodes: {anime.episodes || 'N/A'}</p>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>📡 Status: {anime.status || 'N/A'}</p>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>⭐ Rating: {anime.vote_average || 'N/A'}</p>
                    <p style={{ color: '#999', marginBottom: '8px', fontSize: '0.95rem' }}>🗳️ Votes: {anime.vote_count?.toLocaleString() || 'N/A'}</p>
                    {anime.overview && <p style={{ color: '#bbb', lineHeight: '1.7', marginTop: '15px', fontSize: '0.95rem' }}>{anime.overview}</p>}
            ...
            <ReviewSection
                ...
                contentPoster={anime.poster_path || ''}
            />
        </div>
    )
}
